...
export default function EventDetail() {
  ...
  const similarEventsByNeighborhood = getEventsByNeighborhood(
    event.neighborhood,
    event.id,
  );
  const similarEvents = [
    ...similarEventsByCulture,
    ...similarEventsByNeighborhood,
  ].slice(0, 3);
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-start space-x-4">
            ...
            <div>
              ...
              <div className="text-sm text-[#1e293b]">
                ...
                {!event.accessibility.wheelchairAccess &&
                  !event.accessibility.quietSpace &&
                  !event.accessibility.captions && <div>Limited</div>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-12">
          ...
          <div className="flex border-b border-gray-200 mb-6">
            {(["getting-there", "what-to-expect", "policies"] as const).map(
              (tab) => (
                <button
                  key={tab}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  {event.tags.map((tag, idx) => (
                    <span
                      key={idx}
                      className="inline-flex items-center space-x-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-full text-sm font-medium"
                    >
                      <span>{tag}</span>
                    </span>
                  ))}
              ...
              <div className="bg-green-50 border-l-4 border-green-500 p-6 rounded">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="font-semibold text-[#1e293b]">
                    {event.socialProof.rating.toFixed(1)}
                  </span>
                  <span className="text-gray-600">
                    ({event.socialProof.reviewsCount} reviews)
                  </span>
                </div>
              </div>
            </div>
