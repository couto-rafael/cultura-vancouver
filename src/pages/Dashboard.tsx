...
import { Calendar, MapPin, PlusCircle, Bookmark, Star, Users, TrendingUp, Eye, Heart, Bell, Edit3, Clock, CheckCircle, AlertCircle } from 'lucide-react';
...
const mockNotifications = [
  { id: 1, type: 'success', message: 'Your event "Filipino Heritage Night 2026" was approved!', time: '2 hours ago' },
  { id: 2, type: 'info', message: '47 people saved your event this week.', time: '1 day ago' },
  { id: 3, type: 'warning', message: '"Kulintang Music Workshop" has missing details. Complete your draft.', time: '3 days ago' },
];
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-gray-500 mb-6">Create and manage your cultural events</p>
          <button
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-300 text-sm">{user.email} · Member since {user.joinedDate}</p>
              </div>
            </div>
            <button
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Events Created', value: '3', icon: Calendar, color: 'bg-blue-50', iconColor: 'text-blue-500' },
            { label: 'Total Views', value: '342', icon: Eye, color: 'bg-purple-50', iconColor: 'text-purple-500' },
            { label: 'Events Saved', value: '8', icon: Bookmark, color: 'bg-pink-50', iconColor: 'text-pink-500' },
            { label: 'People Reached', value: '1.2K', icon: Users, color: 'bg-green-50', iconColor: 'text-green-500' },
          ].map(stat => (
            <div key={stat.label} className={`${stat.color} rounded-2xl p-5 flex items-center space-x-3`}>
              <stat.icon className={`w-8 h-8 ${stat.iconColor} flex-shrink-0`} />
              <div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-4">
            ...
            {mockMyEvents.map(event => {
              ...
              return (
                <div key={event.id} className="bg-white rounded-xl shadow-sm p-5 border border-gray-100 hover:shadow-md transition">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <h3 className="font-semibold text-[#1e293b] mb-1">{event.title}</h3>
                      <div className="flex items-center space-x-3 text-sm text-gray-500">
                        ...
                        <span className="text-xs px-2 py-0.5 bg-gray-100 rounded-full">{event.category}</span>
                      </div>
                    </div>
                    <span className={`flex items-center space-x-1 text-xs font-medium px-2.5 py-1 rounded-full ${status.color}`}>
                ...
                {mockNotifications.map(notif => (
                  <div key={notif.id} className="flex items-start space-x-3">
                    <div className="flex-shrink-0 mt-0.5">
                      ...
                      {notif.type === 'info' && <TrendingUp className="w-4 h-4 text-blue-500" />}
              ...
              <div className="space-y-3">
                {mockSavedEvents.map(event => (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-700 group-hover:text-green-600 transition leading-tight line-clamp-2">{event.title}</p>
                      <div className="flex items-center space-x-1 text-xs text-gray-400 mt-0.5">
                        <Calendar className="w-3 h-3" />
                        <span>{new Date(event.date + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                      </div>
                    </div>
                  </div>
                ))}
